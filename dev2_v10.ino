...
int OTA_check_interval = 3600000;//86400000;

//OTA server
String OTA_url = "https://raw.githubusercontent.com/swetha-p95/esp8266ota/main/dev2_v10.bin";
...
void readTemp() {
  ...
  while ((Ta==-127 ) && (T_count < 10) ){
    T_count = T_count+1;
    delay(2000);
    Serial.println("Ta Warning####################");
     DS_TA.requestTemperatures();       // send the command to get temperatures
     tempAmb = DS_TA.getTempCByIndex(0);  // read temperature in °C
      Ta =  tempAmb;
      Serial.println(Ta);
   ...
   T_count = 0; 
  while (( Tf==-127) && (T_count < 10) ){
    T_count = T_count+1;
    delay(2000);
    Serial.println("Tf Warning####################");
    DS_TFridge.requestTemperatures();       // send the command to get temperatures
    tempFridge = DS_TFridge.getTempCByIndex(0);  // read temperature in °C
    Tf =  tempFridge;
    Serial.println(Tf);
